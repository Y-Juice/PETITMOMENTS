...
export async function insertMomentInSupabase(
  ...
  if (authError || !authData.user?.id) {
    return { id: null, error: 'Je bent niet (meer) ingelogd. Log opnieuw in en probeer opnieuw.' }
  }
  ...
  if (error) {
    ...
    return { id: null, error: error.message ?? 'Kon moment niet opslaan in Supabase.' }
  }
